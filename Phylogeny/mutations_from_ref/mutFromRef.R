...
ggplot(df_moreInfos_sub,aes(x=as.Date(date),y=mutations_from_ref,color=as.factor(haplotype)))+geom_point(alpha=1)+scale_color_manual(values=colours_hap[as.factor(df_moreInfos_sub$haplotype)],name = "Haplotype")+t
dev.off()
